import os
import json
import time
import re
from typing import Dict, Any, Optional
from google import genai
from dotenv import load_dotenv

load_dotenv()
GOOGLE_API_KEY = os.getenv("GOOGLE_API_KEY")

if not GOOGLE_API_KEY:
    raise SystemExit("GOOGLE_API_KEY is not set in the environment. Please set it in your .env file.")

client = genai.Client(api_key=GOOGLE_API_KEY)

# Directory paths
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
QUERIES_FILE = os.path.join(SCRIPT_DIR, "metdata.json")
JSON_RESULTS_DIR = os.path.join(SCRIPT_DIR, "outputs")

os.makedirs(JSON_RESULTS_DIR, exist_ok=True)

TECH_GROUPS = [
 "Storage Adapters", "Storage Accessories", "Read-Only Memory", "VPN gateway", "Antivirus",
 "10/100 Hubs & Switches", "10/25/40/50/100 Gigabit Switches", "35mm Camera Lenses",
 "3D Cameras", "3D Design", "3D Printers", "3D Scanners",
 "3G Phones",
 "4G Phones",
 "5G Phones",
 "Accounting",
 "Action Cameras",
 "All-In-One Desktops",
 "AMD Desktops",
 "AMD Notebooks",
 "AMD Video Cards",
 "Amplifiers",
 "Analog Camcorders",
 "Animation",
 "Answering Machines",
 "Apple Desktops",
 "Apple Notebooks",
 "Apple Servers",
 "APS Cameras",
 "Arts & Culture",
 "Assorted Accessories",
 "ATM Cards",
 "ATX Cases",
 "Audio & Video",
 "Audio & Video Equipment Mounts",
 "Audio & Video Switches",
 "Audio Cables",
 "AV Blank Media",
 "AV Receivers",
 "B&W Multifunction Laser Printers",
 "Backgrounds",
 "Banknote Counters",
 "Banknotes/Coins Wrappers",
 "Barcode Scanners",
 "Barebone Systems",
 "Blackberry",
 "Blade Servers",
 "Blu-ray Media",
 "Blu-ray Players",
 "Bluetooth Adapters",
 "Bluetooth Keyboards",
 "Bluetooth Print Servers",
 "Boomboxes",
"Browsers",
"Built-in Car Navigation",
"Bulk Network Cables",
"Business / Economics / Legal",
"Business & Productivity Software",
"Business Notebooks",
"Cable Modems",
"Cabling Accessories",
"CAD/CAM Software",
"Caller ID",
"Camcorder Accessories & Accessory Kits",
"Camcorder Batteries",
"Camcorder Cases",
"Camcorder Cleaning",
"Camcorder Lenses",
"Camcorder Media",
"Camcorder Tripods",
"Camcorders Lens Filters",
"Camera Accessories & Accessory Kits",
"Camera Batteries",
"Camera Cases",
"Camera Cleaning",
"Camera Drones",
"Camera Drones Accessories",
"Camera Lens Filters",
"Camera Tripods",
"Car Accessories",
"Car Amplifiers & Equalizers",
"Car Audio & Video Cables",
"Car Audio Amplifiers",
"Car Audio Crossovers",
"Car Cassette Player",
"Car CD Changers",
"Car CD Player",
"Car DVD Changers",
"Car DVD Players",
"Car GPS",
"Car Monitors",
"Car Speakers",
"Card Readers",
"Carts",
"Cash Registers",
"Cat 5 Cables",
"Cat 5e Cables",
"Cat 6 Cables",
"Cat 6a Cables",
"Cat 7 Cables",
"Cat 8 Cables",
"CD / DVD / Blu-ray Duplicators",
"CD Decks",
"CD Drives",
"CD Media",
"CD/DVD Authoring",
"CD/DVD Printers",
"CD/DVD Wallets",
"CDMA Phones",
"CE Tablets",
"Cellular Phone Accessories",
"Cellular Phone Batteries & Power Adapters",
"Cellular Phone Cables",
"Cellular Phone Cases & Holsters",
"Cellular Phone Prepaid Cards",
"Cellular Phone Screen Protectors",
"Central Speakers",
"CF Cards",
"Chartplotters",
"Chromebooks",
"Cleaning Accessories",
"Cleaning Media",
"Clusters",
"Coaxial Cables",
"Coin Counters",
"Color Laser Printers",
"Color Multifunction Laser Printers",       
"Communication Equipment Books & Manuals",
"Compact AV Systems",
"Compact Film Cameras",
"Computer / Math / Logic",
"Computer Based Training",
"Computer Coolers",
"Computer Fans",
"Computer Hardware Books & Manuals",
"Computer Speakers",
"Conference Microphones",
"Conference Phones",
"Consumer Video Adapters",
"Converters",
"Convertible Notebooks",
"Copiers",
"Copper Transceivers",
"Corded Keyboards",
"Counterfeit Detectors",
"Creativity Software Books & Manuals",
"CRM Software",
"Crossover Cables",
"CRT Monitors",
"CRT TVs",
"Cutters & Trimmers",
"D-AMPS & Analog Cellular Phones",
"Darkroom Accessories",
"Data Analysis & Content Management",
"Data Cartridge Drives",
"Data Collection Terminals",
"Databases & Tools",
"DDR DIMM",
"DDR SO-DIMM ",
"DDR2 DIMM",
"DDR2 SO-DIMM",
"DDR3 DIMM",
"DDR3 SO-DIMM",
"DDR4 DIMM",
"DDR4 SO-DIMM",
"DDR5 DIMM",
"DDR5 SO-DIMM",
"Desktop Accessories",
"Desktop Memory",
"Desktop Publishing",
"Desktop Replacement Notebooks",
"Developer Tools",
"Developing & Processing Equipment",
"Development Tools",
"Dial-Up Modems",
"Digital Camera Lenses",
"Digital Cameras For Kids",
"Digital Projectors",
"Digital Signage Displays",
"Digital Signage Players",
"Digital Voice Recorders",
"DisplayPort Video Adapters",
"DLP Projectors",
"Docking Cradles",
"Document Cameras",
"Document Management Software",
"Document Scanners",
"DOS",
"Dot-Matrix Printers",
"DRAM",
"DSL Modems",
"DVD / HDD Combos",
"DVD / TV Combos",
"DVD Camcorders",
"DVD Changers",
"DVD Decks",
"DVD Drives",
"DVD Media",
"DVD Recorders",
"DVD/VCR Combos",
"DVI Video Adapters",
"DVRs",
"E-commerce Software",
"Earbud & In-Ear Headphones",
"Early Learning",
"eBook Readers",
"eCommerce Solutions",
"Education & Reference",
"Education Software Books & Manuals",
"Email Software",
"Encyclopedia",
"Enlargers & Components",
"Enterprise Bridges & Routers",
"Entertainment Software Books & Manuals",
"EPS Power Supplies",
"Equalizers",
"Ergonomic Keyboards",
"Ethernet Disk Arrays",
"Ethernet Print Servers",
"Extended ATX Cases",
"External Hard Drives",
"External Solid State Drives",
"External Sound Cards",
"External Tape Drives",
"Fanless Coolers & Heatsinks",
"Fax Machines",
"Fax Software",
"Fiber Network Adapters",
"Fibre Channel Disk Arrays",
"File Managers",
"File Transfer",
"Films",
"Finance / Tax Preparation",
"Firewall / VPN Appliances",
"FireWire Adapters",
"FireWire Cables",
"FireWire Disk Arrays",
"Fishfinders",
"Fixed Cellular Phones",
"Flash Duplicators",
"Flashes",
"Flatbed Scanners",
"Floppy Drives",
"FM Transceivers",
"Fonts & Font Tools",
"Foreign Languages & Translation",
"Game Utilities",
"Gamepads",
"Gamer Clothing",
"Gaming Desktops",
"Gaming Keyboards",
"Gaming Notebooks",
"GBIC Transceivers",
"Gigabit Hubs & Switches",
"Gigabit Network Adapters",
"GIS & Map Creation Software",
"GPS Accessories",
"GPS Antennas",
"GPS Cables",
"GPS For Aviation",
"GPS For Cycling",
"GPS For Hiking",
"GPS For Running",
"GPS Modules",
"GPS Power Adapters & Batteries",
"GPS Software",
"GPS Watches",
"Graphic Tablet & Whiteboard Accessories",
"Graphic Tablets & Whiteboards",
"Graphics & Photo Editing",
"Graphics & Publishing Software",
"Groupware",
"GSM Phones",
"Handheld Accessories",
"Handheld Computers",
"Hard Drive Enclosures",
"HD DVD Players",
"HDD Camcorders",
"HDD Duplicators",
"HDMI Cables",
"HDMI Video Adapters",
"HDV Camcorders",
"Headphones Accessories",
"Headphones Cables",
"Headphones Carrying Cases",
"HelpDesk & Inventory Systems",
"High Definition Camcorders",
"Home / Lifestyle Software Books & Manuals",
"Home & Hobbies",
"Home Audio Accessories",
"Home Media Players",
"Hotel & Hospitality TVs & Displays",
"HSE management",
"Human Resources",
"ID Card Printers",
"IDE Cables",
"iDEN Phones",
"InfiniBand Switches",
"Ink-Jet Printers",
"Instant Communication Software",
"Intel Desktops",
"Intel Notebooks",
"Intel Processors",
"Interactive Whiteboards",
"Internal Hard Drives",
"Internal Solid State Drives",
"Internal Sound Cards",
"Internal Tape Drives",
"Internet - E-Commerce Books & Manuals",
"Internet - Web Design / Publishing Books & Manuals",
"Internet / Communication Software Books & Manuals",
"Internet Utilities",
"Intrusion Detection / Prevention",
"IP Telephony",
"IP TV",
"iPhone",
"iPod Accessories",
"iPod Speakers",
"iPods",
"iSCSI Disk Arrays",
"Joysticks",
"Keyboard & Mouse Accessories",
"Keyboard & Mouse Bundles",
"Keyboard & Mouse Cables",
"Keyboard Accessories",
"Kiosks",
"KVM Accessories",
"KVM Cables",
"KVM Console",
"KVM Over IP",
"Label Printers",
"Labeling / Barcode Software",
"Laminators",
"Large Format Camera Lenses",
"Large Format Display Accessories",
"Large-Format Printers",
"Laser Projectors",
"LCD Monitors",
"LCD Projectors",
"LCD TVs",
"LCD/LED Large Format Displays",
"LCOS Projectors",
"Learning Management",
"LED Monitors",
"LED TVs",
"Legacy Cables",
"Legacy Controller Cards",
"Legacy Recording Media",
"Lens Converters & Adapters",
"Light Control",
"Light Meters",
"Lighting Filters",
"Lightning Cables",
"Linux",
"Linux Software",
"Liquid Cooling Systems",
"LNB",
"Long Range Two-Way Radios",
"Long-Zoom Compact Cameras",
"Mac Hardware Books & Manuals",
"Mac Software Books & Manuals",
"MacOS Licenses & Media",
"Magneto-Optical Drives",
"Managed Switches",
"Management Books",
"Map/Atlas/Travel",
"Marine Accessories",
"Marine Cables",
"Marine GPS",
"Medical Displays",
"Medical Keyboards & Mice",
"Medium Camera Lenses",
"Micro ATX Cases",
"Micro-SD Cards",
"Microcassette Recorders",
"Mini ITX Cases",
"Mini-DV Camcorders",
"Mirrorless System Digital Cameras",
"MLC Solid State Drives",
"Mobile Device Management",
"Mobile Device Synchronization",
"Mobile Workstations",
"Modem Accessories",
"Modem Cables",
"Monitor Cables",
"Monitor Stands & Mounts",
"Monochrome Laser Printers",
"Motherboards (for AMD Processors)",
"Motherboards (for Intel Processors)",
"Motion Sensors",
"Motorcycle GPS",
"Mountable Projectors",
"Mounts",
"Mouse Accessories",
"Movies & TV Shows",
"MP3 Players",
"Multi-Purpose Bags",
"Multifunction Ink-Jet Printers",
"Multifunction Page Wide Array Printers",
"Multifunction Supertank Printers",
"Multimedia Players",
"Multimode Fiber Cables",
"Multiplexers",
"Music",
"NAS",
"Natural Sciences",
"Netbooks",
"Network & Enterprise Management",
"Network Antennas & Accessories",
"Network Appliance Software",
"Network Audio Components",
"Network Cabling Accessories",
"Network Cameras",
"Network Hubs",
"Network Security Appliances",
"Network Security Books & Manuals",
"Network Security Software",
"Network Software",
"Network Utilities",
"Networking Hardware Books & Manuals",
"Networking Memory",
"Networking Software Books & Manuals",
"Notebook Batteries",
"Notebook Docking Stations",
"Notebook Hard Drives",
"Notebook Power Adapters/Chargers",
"Numeric Keypads",
"NVIDIA Video Cards",
"OCR Software",
"Office Application Suites",
"Office Software",
"OLED TVs",
"Operating System Books & Manuals",
"Optical Cables",
"Optical Transceivers",
"OS Suites",
"Outdoor Speakers",
"Outdoor TVs",
"Output Accessories",
"Over-Ear & On-Ear Headphones",
"Overhead Projector Supplies",
"Overhead Projectors",
"Page Wide Array Printers",
"Parallel Cables",
"Patch Cables",
"Patch Panels",
"PBX Phones & Systems",
"PC Card Network Adapters",
"PC Maintenance",
"PC Media Centers",
"PCI Network Adapters",
"PCI-E Network Adapters",
"PCI-X Network Adapters",
"Personal Messengers",
"Phone Accessories",
"Phone/Modem Cables",
"Photo / Video / Sound Libraries",
"Photo / Video Books & Manuals",
"Photo Albums",
"Photo Art Books & Manuals",
"Photo Enlarging Paper",
"Photo Frames",
"Photo Printers",
"Photo Storage",
"Photographic Chemistry",
"Picture Books",
"Plasma Panels",
"Plasma TVs",
"Point Of Sale Computers",
"Policy Management",
"Portable Cassette Players & Recorders",
"Portable CD Players",
"Portable DVD & Blu-ray Players",
"Portable DVD Players",
"Portable MD Players",
"Portable Media Players",
"Portable Player Accessories",
"Portable Player Cables",
"Portable Player Carrying Cases",
"Portable Player Power",
"Portable Radios",
"Portable Speakers",
"Portable TVs",
"POS Machine Accessories",
"POS Monitors",
"POS Receipt Printers",
"Power Accessories",
"Power Adapters & Chargers",
"Power Cables",
"Power Enclosures",
"Power Over Ethernet Wireless",
"Presentation",
"Print Management",
"Printer Cables",
"Printer Memory",
"Privacy Filters",
"Productivity Software Books & Manuals",
"Professional Camcorders",
"Professional Video Adapters",
"Programming Languages",
"Programming Tools Books & Manuals",
"Project Management",
"Projection Screens",
"Projector Accessories",
"Projector Mounts",
"Projector Trolleys & Stands",
"Public Address Speakers",
"Quad Processors & Video Multiplexers",
"Rack Servers",
"Rack-Mountable Hubs & Switches",
"Rack-Mountable UPS",
"Radio Scanners",
"Radio Tuners",
"Rangefinder Film Cameras",
"RDRAM",
"RDX Media",
"Rear-Projection TVs",
"Recovery / Backup Software",
"Religion",
"Remote Access Software",
"Remote Controls",
"Retail MacOS",
"Retail Windows",
"Right / Left Channel Speakers",
"RISC Servers",
"Rugged Notebooks",
"SAN",
"SAS Cables",
"SATA Cables",
"SATA Disk Arrays",
"Satellite & Cable TV Accessories",
"Satellite Car Radio",
"Satellite Dishes",
"Satellite Radio",
"Satellite Receivers",
"Satellite Receivers With HDD",
"Satellite TV Systems",
"Scan Management",
"Scanner Accessories",
"Scanner Power",
"Schedule & Contact Management",
"Screen Protectors",
"SCSI / IDE / Floppy Cables",
"SCSI Cables",
"SCSI Disk Arrays",
"SD Cards",
"SDRAM",
"Security & Privacy Filters",
"Security Cables & Locks",
"Security Cameras",
"Security Chips, Tokens & Smart Cards",
"Security DVRs",
"Serial ATA Cables",
"Serial Cables",
"Server Accessories",
"Server Hard Drives",
"Server Memory",
"Server/Workstation Motherboards",
"SFP Transceivers",
"SFX Power Supplies",
"Sheetfed Scanners",
"Short Range Two-Way Radios",
"Short-Throw Projectors",
"Shredders",
"Signal Extenders",
"Single-board Computers",
"Singlemode Fiber Cables",
"SLC Solid State Drives",
"Slide / Film Scanners",
"SLR Digital Cameras",
"SLR Film Cameras",
"Smart Glasses",
"Smart Tracking",
"Smart Watch Accessories",
"Smart Watches",
"SmartCard Readers",
"Software Suites",
"SOHO Bridges & Routers",
"SOHO Hubs & Switches",
"Sound Bars",
"Sound Editing & Recording",
"Spare Parts",
"Speaker Accessories",
"Speaker Cables",
"Speaker Support Hardware",
"Speakerphones",
"Special Network Cables",
"Special Scanners",
"Specialized Network Devices",
"Specialty Film Cameras",
"Specific Cables",
"Specific Cases",
"Spreadsheets",
"Stand-Alone UPS",
"Stands",
"Stands for Notebooks & Tablets",
"Stationary DVD Players",
"Studio Light",
"Studio Mounting",
"Subwoofers",
"Supertank Printers",
"Surge Suppressors & PDUs",
"Surround Speakers",
"Switch Expansion Modules",
"System Deployment & Migration Software",
"Tablet Carrying Cases",
"Tablet Computers",
"Tablet PCs",
"Tape Automation",
"Tape Decks",
"Telephone Accessories",
"Thermal Printers",
"Thin Client Notebooks",
"Thin Clients",
"Thunderbolt Disk Arrays",
"Time Lapse VCR",
"Tools & Tool Kits",
"Touchscreen Monitors",
"Tower Servers",
"Traffic Balancers & Optimizers",
"True Wireless Earbuds",
"Turntable Accessories",
"Turntables",
"TV & Radio Antennas",
"TV accessories",
"Ultra Thin Notebooks",
"Ultra-Light Notebooks",
"Ultra-Mobile PCs",
"Ultrabooks",
"Underwater Digital Cameras",
"Underwater Film Cameras",
"Unix",
"Unix Software",
"Unmanaged Switches",
"Upconverting DVD Players",
"UPS Batteries",
"Upscaling DVD Players",
"USB Cables",
"USB Controllers",
"USB Disk Arrays",
"USB Hubs",
"USB Network Adapters",
"Utilities",
"Utilities Software Books & Manuals",
"Utilities Suites",
"Video & Audio Conferencing",
"Video Cables",
"Video Capture Cards",
"Video Controller Memory Upgrades",
"Video Converters",
"Video Editing",
"Video Editing Controllers, Mixers & Titlers",
"Video Games & Gadgets",
"Video Servers",
"Video Surveillance Accessories",
"Video Surveillance Solutions",
"Virtual Reality Cameras",
"Virtualization Software",
"Voice / Data Modules",
"Voice Recognition / Text-to-Speech",
"VoIP Gateways",
"VoIP Modules",
"VoIP Phones",
"VoIP Routers",
"VoIP Software",
"VR Headsets for Smartphones",
"Watch Phones",
"Web Cameras",
"Web Conferencing Software",
"Web Design / Publishing",
"Web Development",
"Wheels & Pedals",
"Windows Licenses & Media",
"Windows To Go drives",
"Wired Headphones",
"Wired Network Adapters",
"Wired Telephones",
"Wireless Access Points",
"Wireless Audio & Video Sharing",
"Wireless Bridges",
"Wireless Controllers",
"Wireless Headphones",
"Wireless IP Cameras",
"Wireless Keyboards",
"Wireless Network Adapters",
"Wireless NICs",
"Wireless Print Servers",
"Wireless Routers",
"Wireless Telephones",
"Word Processing",
"Workstation Blade",
"x86 Blade Servers",
"x86 Rack Servers",
"x86 Tower Servers",
"XD Cards",
"10/100 Network Adapters",
"AMD Processors",
"Anti-Spam",
"Anti-Spyware",
"Antivirus & Security Software",
"ATX Power Supplies",
"Authentication Software",
"Backpacks",
"Blu-ray Drives",
"Bluetooth Mice",
"Briefcases",
"Brother Ink Cartridges",
"Brother Toner Cartridges",
"Calculator Accessories",
"Canon Ink Cartridges",
"Canon Toner Cartridges",
"Cash Drawers",
"Cellular Modems",
"Computerized Time Clock Systems",
"Corded Mice",
"Dell Ink Cartridges",
"Dell Toner Cartridges",
"Desktop Calculators",
"Electronic Dictionaries",
"Electronic Organizers",
"Encryption Software",
"Epson Ink Cartridges",
"Epson Toner Cartridges",
"Financial Calculators",
"Firewalls",
"Flash Memory Camcorders",
"Gaming Mice",
"Graphing Calculators",
"Hard Drive Mounting",
"HP Ink Cartridges",
"HP Toner Cartridges",
"Internet Filtering Software",
"Intrusion Detection/Prevention",
"ISDN Modems",
"Kodak Ink Cartridges",
"Konica Minolta Toner Cartridges",
"KVM Switches",
"Leather Carrying Cases",
"Lexmark Ink Cartridges",
"Lexmark Toner Cartridges",
"Messenger Bags",
"Modem / NIC Combos",
"Notebook Memory",
"Notebook Mice",
"Notebook Sleeves",
"OKI Toner Cartridges",
"Other Ink Cartridges",
"Other Toner Cartridges",
"Panasonic Toner Cartridges",
"Pocket Calculators",
"Pocket Projectors",
"Power Banks",
"Printing Calculators",
"Projector Cables",
"Projector Lamps",
"Projector Power",
"Projector Screens",
"Samsung Ink Cartridges",
"Samsung Toner Cartridges",
"SAS Disk Arrays",
"Scientific Calculators",
"Security Cabinets",
"Security Suites",
"Sharp Ink Cartridges",
"Sharp Toner Cartridges",
"Standalone Time Clock Machines",
"Styli",
"Tape Media",
"Time Clock Machines Accessories",
"Touchscreen Large Format Displays",
"Typewriter Accessories",
"Typewriters",
"Video Surveillance Software",
"VPN Software",
"VR Headsets",
"Wheeled Cases",
"Wired IP Cameras",
"Wireless Mice",
"karangroup2",
"karansubgroup1",
"Mini Tablet",
"Large tablet",
"6 Tablet",
"subgroup1",
"subgroup2",
"super subgroup1",
"super subgroup2",
"subgroup7",
"subgroup5",
"Techgroup 3",
"Tech group 3",
"Test Tech Group 1",
"Legal",
"News",
"Distributed Storage (CDN)",
"Design and Development",
"Testing",
"Service Desk",
"IT Service Management",
"Business Continuity and Disaster Recovery",
"Data Management",
"Data Warehouse",
"Machine Learning",
"Artificial Intelligence",
"Streaming",
"Test Group",
"Application Gateway Standard v2",
"Azure Active Directory B2X",
"Azure App Service Basic Plan",
"Azure App Service Basic Plan - Linux",
"Azure App Service Free Plan",
"Azure App Service Free Plan - Linux",
"Azure App Service Premium v3 Plan - Linux",
"Azure Bastion",
"Azure DNS",
"Azure Monitor",
"Backup",
"Bandwidth Inter-Region",
"Blob Storage",
"Files",
"Fortinet FortiGate Next-Generation Firewall (VM)",
"Functions",
"General Block Blob",
"General Block Blob v2",
"IoT Hub",
"IoT Hub Device Provisioning",
"IP Addresses",
"Key Vault",
"Load Balancer",
"Log Analytics",
"Logic Apps",
"Microsoft Defender CSPM",
"Microsoft Defender for Resource Manager",
"Microsoft Defender for Servers",
"Network Watcher",
"Premium SSD Managed Disks",
"Process Automation",
"Queues",
"Queues v2",
"Rtn Preference: MGN",
"Service Bus",
"SQL Database Single Standard",
"SQL Database Single/Elastic Pool General Purpose - Compute Gen5",
"SQL Database Single/Elastic Pool General Purpose - SQL License",
"SQL Database Single/Elastic Pool General Purpose - Storage",
"SQL Server Express Edition",
"Standard HDD Managed Disks",
"Standard Page Blob",
"Standard Page Blob v2",
"Standard SSD Managed Disks",
"Storage - Bandwidth",
"Tables",
"Ubuntu 24.04",
"Virtual Machines BS Series",
"Virtual Machines BS Series Windows",
"Virtual Machines Dav4 Series Windows",
"Virtual Machines DCSv2 Series",
"Virtual Machines DCSv2 Series Windows",
"Virtual Machines Ddsv5 Series Windows",
"Virtual Machines Dlsv5 Series",
"Virtual Machines Dsv4 Series",
"Virtual Machines Dv2 Series",
"Virtual Machines Dv2 Series Windows",
"Virtual Machines Dv3 Series",
"Virtual Machines Dv3 Series Windows",
"Virtual Machines F Series Windows",
"Virtual Machines FSv2 Series",
"Virtual Machines FSv2 Series Windows",
"Virtual Network Private Link",
"Cloud VPN Gateway",
"Zabbix Proxy 6.4",
"Zabbix Server 6.4",
"AWS Backup",
"Amazon EC2 Container Registry (ECR)",
"Amazon Elastic Compute Cloud - Compute",
"Amazon Elastic Container Service",
"Amazon Elastic Container Service for Kubernetes",
"Amazon Elastic File System",
"Amazon Elastic Load Balancing",
"Amazon ElastiCache",
"Amazon GuardDuty",
"Amazon Macie",
"Amazon Managed Service for Prometheus",
"Amazon OpenSearch Service",
"Amazon Pinpoint",
"Amazon Relational Database Service",
"Amazon Simple Email Service",
"Amazon Simple Notification Service",
"Amazon Simple Queue Service",
"Amazon Simple Storage Service",
"Amazon Virtual Private Cloud",
"Amazon WorkDocs",
"Amazon WorkSpaces",
"AmazonCloudWatch",
"AWS CloudFormation",
"AWS CloudTrail",
"AWS Config",
"AWS Cost Explorer",
"AWS Glue",
"AWS Key Management Service",
"AWS Lambda",
"AWS Secrets Manager",
"AWS Security Hub",
"AWS Step Functions",
"AWS Support (Business)",
"AWS Systems Manager",
"AWS X-Ray",
"CloudWatch Events",
"EC2 - Other",
"Keycloak with MySQL on Ubuntu 20.04 LTS by Neodoo",
"Pinecone Vector Database - Pay As You Go Pricing",
"Tax",
"Pinecone - Pay As You Go",
"Amazon Elastic CDDDD"
]



TECH_TYPES = [
    "Hardware", "License", "Subscription", "Maintenance", 
    "Virtual Machines", "Freeware", "Certificate"
]

def create_specification_prompt(item: Dict[str, Any]) -> str:
    """Create a prompt to fill specifications based on category schema."""
    item_str = json.dumps(item, indent=2)

    prompt = f"""Create a JSON schema using the provided raw product data. 
    Use web search to find correct information if the provided data seems incorrect or incomplete,
    If the model name contains "x", treat it as a series placeholder (not a full model).  
    Identify real models in that series and return representative specs common across them. 
    Fill the data from web search if null in product data
    Product Data:
    {item_str}

    Create a JSON object with the following structure:

    - ProductDescription: Create using manufacturer and model name
    - Brand: Include subjson with name, address, website
    - ProductFeatures depending on category:
        • Notebook / Laptop: cpu, ram, storage, display, gpu, network, model_number, serial_number
        • Desktop / Mini PC: cpu, ram, storage, gpu, network, model_number, serial_number
        • Monitor / Display: display, panel_type, refresh_rate, response_time, brightness, ports, model_number, serial_number
        • Storage Device (SSD / HDD): storage, interface, form_factor, read_speed, write_speed, model_number, serial_number
        • Network Device (Router / Adapter): type, interface, band, speed, wireless_standard, model_number, serial_number
        • Peripheral (Keyboard / Mouse / Accessory): type, connection, compatibility, power, model_number, serial_number
        • Other: include relevant specs in similar key-value structure.

    - TechGroup: Select from available tech groups
    - TechType: Select from available tech types  

    Available Tech Groups: {', '.join(TECH_GROUPS)}
    Available Tech Types: {', '.join(TECH_TYPES)}

    Instructions:
    - Fill in all fields with accurate information from the product data and web search
    - Return the response in the exact JSON format.
    - Do not include any explanatory text outside the JSON."""

    return prompt

def safe_filename(manufacturer: str, model: str) -> str:
    """Create a safe filename from manufacturer and model."""
    # Clean manufacturer and model for filename
    clean_manufacturer = re.sub(r'[\\/:"*?<>|]+', "_", manufacturer)
    clean_model = re.sub(r'[\\/:"*?<>|]+', "_", model)
    return f"{clean_manufacturer}_{clean_model}.json"

def extract_json_string(text: str) -> str:
    """Extract a JSON string from LLM text which may include markdown fences."""
    # 1) Try fenced code block with json
    fenced = re.findall(r"```json\s*([\s\S]*?)\s*```", text, flags=re.IGNORECASE)
    if fenced:
        candidate = fenced[0].strip()
        return candidate

    # 2) Try any fenced block without language
    fenced_any = re.findall(r"```\s*([\s\S]*?)\s*```", text)
    for block in fenced_any:
        block_stripped = block.strip()
        if block_stripped.startswith("{"):
            return block_stripped

    # 3) Heuristic: try parsing from each '{'
    brace_indices = [m.start() for m in re.finditer(r"\{", text)]
    for start in brace_indices:
        snippet = text[start:]
        close_indices = [m.end() for m in re.finditer(r"\}", snippet)]
        for end in reversed(close_indices):
            candidate = snippet[:end]
            candidate_stripped = candidate.strip()
            if candidate_stripped:
                try:
                    json.loads(candidate_stripped)
                    return candidate_stripped
                except Exception:
                    continue

    # Fallback: return the whole text
    return text.strip()

def process_products(item: Dict[str, Any], model: str = "gemini-2.5-flash") -> None:
    """Process a single item with specification filling."""
    
    manufacturer = item.get('hardware_data', {}).get('manufacturer', 'Unknown')
    model_name = item.get('hardware_data', {}).get('model', 'Unknown')
    
    print(f"Processing: {manufacturer} {model_name}")

    try:
        spec_prompt = create_specification_prompt(item)
        start_time = time.time()
        
        # Updated API call for newer version
        response = client.models.generate_content(
            model=model,
            contents=spec_prompt,
            config={
                "response_mime_type": "application/json",
            }
        )
        response_time = time.time() - start_time
        
        raw_text = response.text if hasattr(response, "text") else str(response)
        json_str = extract_json_string(raw_text)
        
        try:
            json_obj = json.loads(json_str)
        except Exception as e:
            print(f"Failed to parse JSON for {manufacturer} {model_name}: {e}")
            print(f"Raw response: {raw_text[:200]}...")
            # Save raw response for debugging
            debug_filename = safe_filename(manufacturer, model_name + "_debug")
            debug_path = os.path.join(JSON_RESULTS_DIR, debug_filename)
            with open(debug_path, "w", encoding="utf-8") as f:
                f.write(raw_text)
            return

        # Save the filled data to results
        filename = safe_filename(manufacturer, model_name)
        output_path = os.path.join(JSON_RESULTS_DIR, filename)

        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(json_obj, f, ensure_ascii=False, indent=2)

        print(f"Saved to {output_path} ({response_time:.2f}s)")

    except Exception as e:
        print(f"Error processing {manufacturer} {model_name}: {e}")

def main():
    """Main function to process all queries."""
    # Load queries
    if not os.path.exists(QUERIES_FILE):
        print(f"Queries file not found: {QUERIES_FILE}")
        return
    
    try:
        with open(QUERIES_FILE, 'r', encoding='utf-8') as f:
            items = json.load(f)
    except Exception as e:
        print(f"Error loading queries file: {e}")
        return
    
    # Ensure items is a list
    if not isinstance(items, list):
        print("Error: Expected a list of items in the JSON file")
        return
    
    print(f"Processing {len(items)} items...")
    
    for i, item in enumerate(items, 1):
        print(f"\n--- Item {i}/{len(items)} ---")
        process_products(item)
        
        # Add a small delay between requests to avoid rate limiting
        if i < len(items):
            time.sleep(2)
    
    print(f"\nCompleted processing {len(items)} items.")
    print(f"Results saved to: {JSON_RESULTS_DIR}")

if __name__ == "__main__":
    main()